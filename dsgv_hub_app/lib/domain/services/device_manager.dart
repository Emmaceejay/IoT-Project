...
class DeviceManager extends AsyncNotifier<List<IoTDevice>> {
  ...
  @override
  Future<List<IoTDevice>> build() async {
  ...
  Future<void> sendCommand(String deviceId, Map<String, dynamic> command) async {
    ...
    final device = state.value?.firstWhere(
      ...
      orElse: () => IoTDevice(uniqueDeviceId: deviceId, deviceName: ''),
    );
  ...
  Future<void> handleAnnounce(IoTDevice announced) async {
    final devices = state.value ?? [];
...
final deviceManagerProvider =
    AsyncNotifierProvider<DeviceManager, List<IoTDevice>>(DeviceManager.new);
