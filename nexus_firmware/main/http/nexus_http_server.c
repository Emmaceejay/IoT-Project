/**
 * nexus_http_server.c — Local HTTP Transport (Tasmota-Compatible REST API)
 *
 * Activated when the Nexus Hub App detects it is on the same Wi-Fi network
...
static const char *TAG = "nexus_http";
...
esp_err_t nexus_http_server_start(void) {
...
void nexus_http_server_stop(void) {
    if (s_server) {
        httpd_stop(s_server);
        s_server = NULL;
...
static esp_err_t handle_cmd_post(httpd_req_t *req) {
    ...
    apply_capability(cap->valuestring, val);
    cJSON_Delete(root);
    ...
    ESP_LOGI(TAG, "POST /api/cmd body=%s", body);
    return ESP_OK;
}
...
/**
 * Applies a Nexus capability command to g_device_state and drives GPIO.
