/**
 * nexus_gpio.c — GPIO, LEDC PWM, and sensor layer
 *
 ...
 * A background FreeRTOS task reads sensors every NEXUS_TELEMETRY_INTERVAL_MS
 * and publishes a telemetry snapshot via nexus_mqtt_publish_telemetry().
...
static void build_telemetry(char *buf, size_t len) {
    STATE_LOCK();
    nexus_device_state_t s = g_device_state;
    STATE_UNLOCK();
...
static void sensor_task(void *pvParam) {
    ...
    for (;;) {
        ...
        ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(NEXUS_TELEMETRY_INTERVAL_MS));
        ...
        nexus_mqtt_publish_telemetry(buf);
    }
}
...
void nexus_gpio_init(void) {
    ...
    xTaskCreate(sensor_task, "nexus_sensors", 4096, NULL, 2, &s_sensor_task_handle);
...
void nexus_gpio_dimmer_set(int pct) {
    if (pct < 0) pct = 0;
    if (pct > 100) pct = 100;
...
void nexus_gpio_ct_set(int kelvin) {
...
void nexus_gpio_rgb_set(uint8_t r, uint8_t g, uint8_t b) {
...
void nexus_gpio_apply_state(void) {
    ...
    if (s.relay_states[0] && s.color_temp_k > 0) {
        nexus_gpio_ct_set(s.color_temp_k);
    } else {
    ...
    if (s.relay_states[0]) {
        nexus_gpio_rgb_set(s.rgb_r, s.rgb_g, s.rgb_b);
    } else {
        nexus_gpio_rgb_set(0, 0, 0);
    }
}
