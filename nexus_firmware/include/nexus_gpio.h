...
/**
 * nexus_gpio.h — Public GPIO/hardware layer API
 *
...
// Must be called once, after nexus_mqtt_start() (needs g_state_mutex).
void nexus_gpio_init(void);

// ── Relay outputs (capabilities: "relay" … "relay_4") ────────────────────────
// nexus_gpio_relay_set() controls gang 0 (the primary relay), e.g. from a
// physical button handler. MQTT / HTTP commands use nexus_gpio_apply_state()
// which drives all gangs from g_device_state.relay_states[] at once.
void nexus_gpio_relay_set(bool on);   // gang 0 convenience wrapper
bool nexus_gpio_relay_get(void);

// ── Dimmer (capability: "dimmer") ────────────────────────────────────────────
void nexus_gpio_dimmer_set(int pct);    // 0-100 %

// ── Color temperature (capability: "color_temperature") ──────────────────────
void nexus_gpio_ct_set(int kelvin);     // 2000-6500 K — blends warm/cool PWM

// ── RGB light (capability: "rgb_light") ──────────────────────────────────────
void nexus_gpio_rgb_set(uint8_t r, uint8_t g, uint8_t b);  // 0-255 per channel
