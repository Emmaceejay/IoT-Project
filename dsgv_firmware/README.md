# DSGV Firmware — ESP32 Dual-Stack (Matter + MQTT)

## Overview
This workspace contains the C/C++ firmware for all DSGV Hub-compatible ESP32 devices.
It runs two communication stacks simultaneously:
1. **Matter (CHIP SDK)** — handles local commissioning via Apple/Google/Alexa
2. **MQTT Client** — reports telemetry to DSGV Hub app via EMQX or local Mosquitto
...
The `dsgv_config.h` file acts as the single source of truth for the device identity. The app reads the `capabilities` JSON payload reported by the device to automatically adjust its UI.
To build a thermostat instead of a light switch:
1. Change the GPIO pins in `dsgv_config.h`.
...
3. Update the MQTT telemetry payload in `dsgv_mqtt.c` to send `{"capabilities": ["hvac_control", "temperature_sensor"]}`.
