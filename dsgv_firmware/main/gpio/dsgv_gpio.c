...
static void switch_task(void *pvParam) {
    ...
    for (;;) {
        ...
        // Drain contact-bounce events arriving within the next 50 ms
        ...
        // Edge-triggered toggle: relay state is independent of switch position.
        STATE_LOCK();
...
void DSGV_gpio_init(void) {
    ...
    // ── Wall switch inputs (latch switch, both-edge toggle) ────────────────
