...
/**
 * DSGV_provisioning.h
 * BLE GATT-based Wi-Fi provisioning for DSGV Hub firmware.
 *
 * Provisioning flow (triggered on first boot or after factory reset):
 *
 *   1. Firmware calls DSGV_provisioning_start() when no Wi-Fi
 *      credentials are found in NVS.
 *
 *   2. Device starts a NimBLE GATT server advertising as "DSGVHub_XXXXXX"
 ...
 *
 *   4. DSGV Hub App (Flutter) connects via BLE, discovers the provisioning
